...
<sub><code>azure</code> &nbsp;·&nbsp; <code>backup</code> &nbsp;·&nbsp; <code>recovery</code></sub>
...
> [!TIP]
> **One-line install** — `/plugin install azure-backup-recovery@claude-m-microsoft-marketplace`
...
| Component | Items |
|---|---|
| **Commands** | `/backup-cross-region-check` · `/backup-job-health` · `/backup-recovery-plan-audit` · `/backup-recovery-setup` · `/backup-restore-drill` |
| **Agents** | `azure-backup-recovery-reviewer` |
| **Skills** | `azure-backup-recovery` |
...
```text
Use azure-backup-recovery to audit and operate Azure resources end-to-end.
```
...
flowchart LR
    ...
    U["You"]:::user
    CC["Claude Code"]:::cc
    PG["azure-backup-recovery<br/>(plugin)"]:::plugin
...
<details>
<summary><b>Composable stacks that include <code>azure-backup-recovery</code></b></summary>
