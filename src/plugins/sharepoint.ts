    ...
    try {
      switch (toolName) {
        ...
        case "sharepoint_upload_file": {
          const { siteId, driveId, parentFolderId, fileName, content, mimeType } =
            UploadFileArgs.parse(args);
        ...
        case "sharepoint_download_file": {
          ...
          const data = await this.graphGet(url) as { "@microsoft.graph.downloadUrl"?: string };
          return this.ok({
