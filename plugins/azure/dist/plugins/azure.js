...
// Helpers
// ---------------------------------------------------------------------------
function resourceUrl(subscriptionId, resourceGroup, provider, resourceType, resourceName, apiVersion) {
...
function rgLockBase(subscriptionId, resourceGroup) {
    if (resourceGroup) {
        ...
        try {
            switch (toolName) {
                ...
                case "azure_get_resource": {
                    const { subscriptionId, resourceGroup, provider, resourceType, resourceName, apiVersion } = GetResourceArgs.parse(args);
                ...
                case "azure_create_or_update_resource": {
                    const { subscriptionId, resourceGroup, provider, resourceType, resourceName, apiVersion, body } = CreateOrUpdateResourceArgs.parse(args);
                ...
                case "azure_delete_resource": {
                    const { subscriptionId, resourceGroup, provider, resourceType, resourceName, apiVersion } = DeleteResourceArgs.parse(args);
                    const url = resourceUrl(subscriptionId, resourceGroup, provider, resourceType, resourceName, apiVersion ?? API_GENERAL);
                    await this.graphDelete(url);
                    return this.ok({ deleted: true, resourceName });
                }
                case "azure_move_resources": {
                    const { subscriptionId, sourceResourceGroup, targetResourceGroupId, resourceIds } = MoveResourcesArgs.parse(args);
                ...
                case "azure_create_role_assignment": {
                    const { scope, principalId, roleDefinitionId, roleAssignmentId } = CreateRoleAssignmentArgs.parse(args);
                ...
                case "azure_update_resource_tags": {
                    const { subscriptionId, resourceGroup, provider, resourceType, resourceName, apiVersion, tags, operation } = UpdateResourceTagsArgs.parse(args);
                    const url = resourceUrl(subscriptionId, resourceGroup, provider, resourceType, resourceName, apiVersion ?? API_GENERAL);
                ...
                case "azure_validate_deployment": {
                    const { subscriptionId, resourceGroup, deploymentName, template, parameters } = ValidateDeploymentArgs.parse(args);
                ...
                case "azure_create_deployment": {
                    const { subscriptionId, resourceGroup, deploymentName, template, parameters, mode } = CreateDeploymentArgs.parse(args);
